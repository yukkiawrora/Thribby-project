        ...
        if(!empty($login)){
            if($login->active == 1){
                ...
                if($login->status==1)
                {
                    ...
                    $login_detail = array('FirstName'=>$login->first_name,'LastName'=>$last_name,'image'=>$image,'PhoneNumber'=>$login->mobile_number,'UserID'=>$login->entity_id,'notification'=>$login->notification,'rating'=>$review,'phone_code'=>$login->phone_code);
