...
class Order extends CI_Controller { 
    ...
    public function ajaxview() {
        ...
        foreach ($grid_data['data'] as $key => $val) {
            ...
            $accept = ($val->status != 1 && $val->restaurant_id)?'<button onclick="disableDetail('.$val->entity_id.','.$val->restaurant_id.','.$val->entity_id.')"  title="Click here for accept order" class="delete btn btn-sm danger-btn margin-bottom"><i class="fa fa-check"></i> Accept</button>':'';
            $records["aaData"][] = array(
                ...
                ' '.$accept.'<button onclick="deleteDetail('.$val->entity_id.')"  title="Click here for delete" class="delete btn btn-sm danger-btn margin-bottom"><i class="fa fa-times"></i> Delete</button> <button onclick="getInvoice('.$val->entity_id.')"  title="Click here for Download Invoice" class="delete btn btn-sm danger-btn margin-bottom"><i class="fa fa-times"></i> Invoice</button>
                    <button onclick="updateStatus('.$val->entity_id.','.$ostatus.','.$val->user_id.')" '.$disabled.' title="Click here for update status" class="delete btn btn-sm danger-btn margin-bottom"><i class="fa fa-edit"></i> Change Status</button><button onclick="statusHistory('.$val->entity_id.')" title="Click here for view status history" class="delete btn btn-sm danger-btn margin-bottom"><i class="fa fa-history"></i> Status History</button>
                    '
            );
            $nCount++;
        }        
